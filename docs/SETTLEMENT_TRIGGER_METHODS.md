...
**结算逻辑：**
- ✅ **测试用户**（test001/test002）：立即结算所有 `confirmed` 订单（0天冷静期）
- ✅ **普通用户**：只结算确认后**超过冷静期**（默认15天）的订单
- ❌ **未满冷静期的订单**：不会被结算，继续等待

**示例：** 今天是 2025-12-04，冷静期 15 天
- 订单 A（2025-11-10 确认）→ ✅ 会结算（超过15天）
- 订单 B（2025-12-03 确认）→ ❌ 不结算（仅1天，需再等14天）
- test001 订单（任何时间）→ ✅ 立即结算（0天冷静期）
